...
// Input: none
// Output: none
extern uint32_t Prewait ;                   // loops between Clock_Init48MHz() called and PCM idle (expect 0)
extern uint32_t CPMwait;                   // loops between Power Active Mode Request and Current Power Mode matching requested mode (expect small)
extern uint32_t Postwait;                  // loops between Current Power Mode matching requested mode and PCM module idle (expect about 0)
extern uint32_t IFlags;                    // non-zero if transition is invalid
extern uint32_t Crystalstable;             // loops before the crystal stabilizes (expect small)
